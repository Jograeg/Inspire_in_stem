#!/usr/bin/python

#calculate area of a circle, volume and surface area of a cylinder and volume of a cube  (get user input)
PI = 3.142
radius = input("enter the radius of the circle")
radius = 7
area_of_a_circle = PI * int(radius) * int(radius)
print(" the area of the circle is? " + str(area))


# volume of a cylinder
height = input("enter height of the cylinder")
volume = area * int(height)
print("the volume of the cylinderis?" + str(volume))
height = 12


# volume of a cube
lenght = input("enter lenght of cube")
volume = int(lenght) * int(lenght) * int(length)
print("volume of this cube is? " + str(volume))